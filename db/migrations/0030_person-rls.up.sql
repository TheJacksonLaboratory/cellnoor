...
create or replace function current_person_can_read_all_projects() returns boolean language plpgsql volatile strict as $$
    declare
        current_user_id uuid = current_user::uuid;
        can_read boolean;
    begin
        select can_read_all_projects from person_public where id = current_user_id into can_read;
        return can_read;
    end;
$$;

-- If the person we are creating isn't getting can_admin_users, then proceed. If they are, then the
-- current_user also has to have can_admin_users. Same with `can_read_all_projects`
