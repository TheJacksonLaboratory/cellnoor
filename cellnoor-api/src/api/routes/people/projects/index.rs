...
impl api::AuthorizedRequest<Vec<Project>> for (PersonId, ProjectQuery) {
    type ValidationData = ();
...
impl api::Request<Vec<Project>> for (PersonId, ProjectQuery) {
    type Authorized = (PersonId, ProjectQuery);
    type ValidationData = ();
