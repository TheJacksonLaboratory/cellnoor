...
use cellnoor_schema::{chromium_dataset_raw_files, chromium_datasets};
use diesel::prelude::*;
...
async fn select_chromium_dataset_file(
    ...
    let query = chromium_dataset_raw_files::table
        .inner_join(chromium_datasets::table)
        .filter(filter);

    let (content_type, content_encoding, raw_content) = query
        .select((
            chromium_dataset_raw_files::content_type,
            chromium_dataset_raw_files::content_encoding,
            chromium_dataset_raw_files::raw_content,
        ))
    ...
    if content_encoding != "zstd" {
        return Err(db::Error::Other {
            message: "corrupted file".to_owned(),
        });
    }
    ...
    let raw_content = zstd::decode_all(raw_content.as_slice()).map_err(|_| db::Error::Other {
        message: "failed to decompress file".to_owned(),
    })?;
...
where
    chromium_dataset_raw_files::dataset_id: SelectableExpression<QS>,
    chromium_dataset_raw_files::path: SelectableExpression<QS>,
    chromium_datasets::project_id: SelectableExpression<QS>,
{
    let mut filter = BoxedFilter::new_true()
        .and_condition(chromium_dataset_raw_files::dataset_id.eq(id))
        .and_condition(chromium_dataset_raw_files::path.eq(path));
