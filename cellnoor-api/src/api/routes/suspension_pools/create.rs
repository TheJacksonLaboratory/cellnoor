...
async fn insert_suspension_pool_mapping(
    ...
    let mappings: Vec<_> = suspensions
        ...
        .map(|suspension_id| {
            (
                untagged_suspension_pooling::pool_id.eq(pool_id),
                untagged_suspension_pooling::suspension_id.eq(suspension_id),
            )
        })
        .collect();

    diesel::insert_into(untagged_suspension_pooling::table)
        .values(mappings)
        .execute(&mut db_conn)
        .await?;
