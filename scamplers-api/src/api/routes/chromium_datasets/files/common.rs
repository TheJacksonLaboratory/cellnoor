...
fn extract_path(filename: Option<&str>) -> Result<(String, String), ErrorResponse> {
    ...
    let (Some(filename), Some(directory), Some("")) = (
        ancestors.next(),
        ancestors.next(),
    ...
    Ok((directory.to_string(), filename.to_string()))
}
...
mod tests {
    ...
    fn correct_filename() {
        assert_eq!(
            extract_path(Some("parent/file")).unwrap(),
            ("parent".to_owned(), "file".to_owned())
        );
    }
}
