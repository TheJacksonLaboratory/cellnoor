...
function cleanup_docker() {
    docker stop cellnoor-dev >/dev/null
    docker rm cellnoor-dev --volumes >/dev/null
}
...
docker run --name cellnoor-dev --env POSTGRES_PASSWORD=p --publish 5433:5432 --detach postgres:18-alpine

# Thanks ChatGPT
until docker exec --user postgres cellnoor-dev pg_isready >/dev/null 2>&1; do
    sleep 0.1
done
