...
impl BaseSqlStmt {
    ...
    {
        fn map_write_err(e: std::fmt::Error) -> ErrorInner {
            ErrorInner::Other {
                message: e.to_string(),
                sql_state: None,
            }
        ...
        if let Some(limit) = limit {
            write!(stmt, " limit {limit}").map_err(map_write_err)?;
        }
        write!(stmt, " offset {offset}").map_err(map_write_err)?;
...
mod tests {
    ...
    use crate::db::write_where_clause_predicates;
