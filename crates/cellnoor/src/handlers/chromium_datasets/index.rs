...
use cellnoor_schema::{
    cdna::dsl::cdna,
    chip_loadings::{self, dsl::chip_loadings as chip_loadings_table},
    ...
    multiplexing_tags::dsl::multiplexing_tags,
    projects::dsl::projects,
    specimens::{self, table as specimens_table},
    suspension_pooling::dsl::suspension_pooling,
...
pub async fn select_chromium_datasets(
    ...
    let mut chromium_datasets_from_unpooled_specimens = chromium_datasets_to_unpooled_specimens()
        .select(ChromiumDataset::as_select())
        .limit(limit)
        .offset(offset)
        .filter(filter.to_boxed_filter());

    let mut chromium_datasets_from_pooled_specimens = chromium_datasets_to_pooled_specimens()
        .select(ChromiumDataset::as_select())
        .limit(limit)
        .offset(offset)
        .filter(filter.to_boxed_filter());

    let mut stmt = chromium_datasets_from_unpooled_specimens
        .union_all(chromium_datasets_from_pooled_specimens)
        .into_boxed();
    ...
    let datasets = stmt.load(&mut db_conn).await?;
