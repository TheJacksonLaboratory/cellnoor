<script lang="ts">
  ...
  function toSorted(choices: { label: string; value: string }[]) {
    ...
    unique.sort((a, b) => a.label.toLowerCase().localeCompare(b.label.toLowerCase()));

    return unique;
  }
</script>

<label class="label text-base-content font-bold" for={fieldName}>{fieldName}</label>
<div class="join">
  <input
  ...
  <button aria-label="add item" onclick={handleAddition} class="btn btn-primary join-item">
