...
import { DATETIME_FORMATTER } from '$lib/date.js';
  ...
  const { apiTokens, user: {name: userName, email, image, userId} } = $derived(data);
  let apiKeysDialogBox: HTMLDialogElement;

  function oneMonthFromNow() {
    const now = new Date();
    return now.setMonth(now.getMonth());
  }
</script>
  ...
  <dialog bind:this={apiKeysDialogBox} class="modal">
    <div class="modal-box max-w-full xl:max-w-1/2 lg:max-w-3/4">
      <table class="table">
        ...
        <tbody>
          {#each apiTokens as { name, description, created_at, exp }, i}
            <tr>
              <td>
                {name}
              </td>
              ...
              <td>
                  <form method="post" use:enhance action="?/deleteApiToken">
                      ...
                      <input name="apiKeyName" value={name} type="hidden"/>
      ...
      <div class="modal-action flex flex-row justify-evenly">
          <form method="post" use:enhance action="?/createApiToken">
              <label class="input">
                ...
                <input name="apiKeyName" type="text"/>
              </label>
              <label class="input">
                <span class="label">Expires on</span>
                <input name="expiresOn" type="date" value={oneMonthFromNow()} />
              </label>
